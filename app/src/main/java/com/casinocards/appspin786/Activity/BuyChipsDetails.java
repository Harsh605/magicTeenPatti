...
import com.casinocards.appmagicTeenpatti.Comman.DialogSelectPaymentType;

import com.casinocards.appmagicTeenpatti.Comman.PaymentGetway_CashFree;
import com.casinocards.appmagicTeenpatti.Comman.PaymentGetway_Paymt;

import com.casinocards.appmagicTeenpatti.Comman.PaymentGetway_PayuMoney;
...
import com.casinocards.appmagicTeenpatti.UpiActivity;
...
import com.casinocards.appmagicTeenpatti.data.ChipsRepository;
...
public class BuyChipsDetails extends BaseActivity implements PaymentResultListener, PaymentStatusListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView txtCheckStatus = findViewById(R.id.txtCheckStatus);
        txtCheckStatus.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                                setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                    ...
                    }  else if (selected_payment.equalsIgnoreCase(Variables.UPI)) {
                        Toast.makeText(context, "UPI available at this time ", Toast.LENGTH_SHORT).show();
                       callUpiGateway();

                       // callUpiPaymentUrl();
                        //   paymentGetway_payuMoney.startPayment(plan_id);
                    } else if (selected_payment.equalsIgnoreCase(Variables.NewUPI)) {
                      //  Toast.makeText(context, "UPI available at this time ", Toast.LENGTH_SHORT).show();
                     //   callUpiPaymentUrl();
                        //   paymentGetway_payuMoney.startPayment(plan_id);
...
//            }
    ...
    private void callUpiGateway() {
        startActivity(new Intent(BuyChipsDetails.this, UpiActivity.class)
                .putExtra("amount",amount)
                .putExtra("mobile",prefs.getString("mobile", ""))
                .putExtra("name",prefs.getString("name", ""))
                .putExtra("planID",plan_id)
                .putExtra("userId",prefs.getString("user_id", ""))
        );


     //   new DialogWebviewUpi(context).showDialog("");
    }
...
//    }
    ...
    public void orderID() {
        ...
        dialog.setContentView(R.layout.dialog_upi_orderid);
        dialog.setTitle("Title...");
        dialog.setCancelable(true);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
      EditText  edit_OrderID = (EditText) dialog.findViewById(R.id.edit_OrderID);
        ...
        dialog.findViewById(R.id.imglogin).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (edit_OrderID.getText().toString().length() > 0) {
                    String orderID = edit_OrderID.getText().toString();//0000100
                    callUPICheckStatus(orderID);
                    dialog.dismiss();
                } else {
                    Functions.showToast(getApplicationContext(), "Please Enter OrderID");
    ...
    public void callUPICheckStatus( String orderID) {
        ...
        String url="https://indianpay.co.in/admin/payinstatus?order_id="+orderID;
        try {
                    ...
                    try {
                        if (response != null ) {
                            if (response.body() != null ){
                                dialogCheckStaus(response.body());
                        /*        if (!response.body().getStatus().equals("cancelled")) {
Log.d("ZXZ",new Gson().toJson(response.body()));
                                }else {
                                    Log.d("ZXZ",new Gson().toJson(response.body()));
                                    Toast.makeText(BuyChipsDetails.this, response.body().getStatus(), Toast.LENGTH_SHORT).show();
                                }*/
                            }
                        } else {
    ...
    public void callUpiSingleTrans() {
        ServerAPI serverAPI = ApiClient.getServerApi(this);
        //  GeneralUtilities.showDialog(context,"Please wait...");

        try {
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("merchant_id", "04");
                jsonObject.put("merchant_token", "04");
                jsonObject.put("account_no", "cvxcvcv0");
                jsonObject.put("ifsccode", "xcvxv");
                jsonObject.put("amount", amount);
                jsonObject.put("bankname", "name");
                jsonObject.put("remark", "payment");
                jsonObject.put("orderid","orderID");
                jsonObject.put("name", prefs.getString("name", ""));
                jsonObject.put("contact", prefs.getString("mobile", ""));
                jsonObject.put("email", "test@gmai.com");
                Log.d("cvxvc",jsonToBase64(jsonObject));

            JSONObject jsonObject1 = new JSONObject();
            jsonObject1.put("salt", jsonToBase64(jsonObject));

            String json = jsonObject1.toString();
            RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), json);
            Call<UPIPAymentURLBean> call = serverAPI.singleTrans(requestBody);
    ...
    private void dialogCheckStaus(OrderReposeBean body) {
        final Dialog dialog = openDailog(R.layout.dialog_upi_response, 0, BuyChipsDetails.this);
        //    final Dialog dialog = Utitlity.getInstance().openBootmSheetDailog(R.layout.dailog_choose_wpos_option,R.style.AppBottomSheetDialogTheme,getActivity());

        ImageView ivClose = dialog.findViewById(R.id.imgclosetop);
        TextView tvStatus = dialog.findViewById(R.id.tvStatus);
        TextView tvTransID = dialog.findViewById(R.id.tvTransID);
        TextView tvAMT = dialog.findViewById(R.id.tvAMT);
        TextView tvUTR = dialog.findViewById(R.id.tvUTR);
        TextView tvDate = dialog.findViewById(R.id.tvDate);

        tvStatus.setText(body.getStatus());
        tvTransID.setText(body.getTransactionid());
        tvAMT.setText(body.getAmount());
        tvUTR.setText(body.getUtr());
    ...
    public Dialog openDailog(Integer layoutId, int dialogAnim, Activity activity) {
        Dialog dialog = new Dialog(activity, dialogAnim);
        ...
        int width = (int) (size.x * 0.84);
        int height = (int) (size.y * 0.84);
        dialog.show();
